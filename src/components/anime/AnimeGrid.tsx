...
const AnimeGrid = ({ searchFilter, pagiantionOnScroll = true }: Props) => {
	...
	const { data, isLoading, error, isEmptyData } = useAnime<Anime>(searchFilter, page);
	usePageScroll(
		!isLoading && !isEmptyData && pagiantionOnScroll && (searchFilter.maxPages ? searchFilter.maxPages > page : true),
		() => {
			setPage(page + 1);
		},
	...
	return (
		<SimpleGrid
			...
			{isLoading &&
				Array.from({ length: DefaultConfig.searchItemsLimitPerPage }, (_, index) => (
